...
based on device path matching. Depends on RawInputListener for the
low-level Win32 API.
"""
...
class KeyboardRouter:
    ...
    def _resolve_participant(self, device_handle: int) -> int:
        ...
        # Match against known device paths (case-insensitive)
        ...
        if path_lower == p1_lower:
            ...
            logger.debug(f"KeyboardRouter: Handle {device_handle} -> P1")
            return 1
        elif path_lower == p2_lower:
            self._handle_to_participant[device_handle] = 2
            logger.debug(f"KeyboardRouter: Handle {device_handle} -> P2")
            return 2
        else:
            self._handle_to_participant[device_handle] = 0
            return 0
